...
namespace Microsoft.AspNetCore.Hosting
{
    ...
    public static class ListenOptionsHttpsExtensions
    {
        ...
        {
            return listenOptions.UseHttps(new TlsHandshakeCallbackOptions(context =>
            {
                context.AllowDelayedClientCertificateNegotation = true; // TODO: do we still need this enabled by default after adding the new overload?
                return serverOptionsSelectionCallback(context.SslStream, context.ClientHelloInfo, context.State, context.CancellationToken);
            })
            {
